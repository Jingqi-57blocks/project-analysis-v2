...
export class SnapshotNotFoundError extends Error {
  constructor(readonly runId: string | null) {
    super(
      runId === null
        ? "No published analysis exists in this knowledge base. Run `analyze` first."
        : `No published analysis with run id ${runId}. It may have failed before publishing.`,
    );
 ...
 */
export function resolveSnapshot(store: Store, runId?: string): Snapshot {
  const row = runId
    ? store.get<{
        id: number;
        run_id: string | null;
        ...
        `SELECT s.id, s.run_id, s.identity, s.published_at, w.path
         ...
         WHERE s.run_id = ? AND s.published_at IS NOT NULL`,
        [runId],
      )
    : store.get<{
        id: number;
  ...
  if (row === undefined) throw new SnapshotNotFoundError(runId ?? null);
  return {
    id: row.id,
    runId: row.run_id,
...
export class KnowledgeBase {
  ...
  coverageFor(kind: StructuralKind): Coverage {
    const rows = this.store.all<{
      ...
      `SELECT provider_id, language, outcome, reason, record_count
       FROM capability_results WHERE snapshot_id = ? AND kind = ?
       ORDER BY provider_id, language`,
...
export function openKnowledgeBase(store: Store, runId?: string): KnowledgeBase {
  return new KnowledgeBase(store, resolveSnapshot(store, runId));
}

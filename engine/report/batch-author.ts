...
function promptPolicyVersion(blockId: string): string {
  if (blockId === "module-flows-branches.flows" || blockId === "project-roles-flows.paths") return "flow-policy.v4";
...
function boundedLifecycleFacts(request: AuthoringRequest): readonly CitedFact[] {
  ...
  return [...new Map(
    [...coreFlows, ...states, ...signals, ...excerpts, ...labels].map((fact) => [fact.factId, fact] as const),
  ).values()];
}
...
function validateBatch(requests: readonly AuthoringRequest[], response: BatchResponse): readonly string[] {
  ...
  for (const task of response.tasks) {
    ...
    if (lifecycleTask(request)) {
      ...
      const lifecycleKinds = new Set(["scheduled-task", "state", "state-transition", "value-set", "condition", "decision", "guard", "business-rule", "validation-rule"]);
      const required = bounded.filter((fact) => lifecycleKinds.has(fact.kind)).map((fact) => fact.factId);

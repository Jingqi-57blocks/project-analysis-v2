...
export interface DataAccessRecord {
  ...
  /** The entry / trace step / decision this effect is triggered by, when known. */
...
/**
 * Every data access keeps a definite operation kind, a citation, and — for a
 * resolved effect — a trigger; a read is never a write. The counts reconcile with
 * the accesses, so nothing is dropped or miscounted.
 */
...
export function validateInteractions(set: InteractionSet): ContentValidation {
  ...
  for (const i of set.interactions) {
    ...
    // An unresolved target must carry a trigger or a target string to be locatable.
    if (i.resolution === "unresolved" && i.target.length === 0 && i.triggeredBy === null) {
      reasons.push(`interaction ${i.id} is unresolved with no target or trigger`);
    }
  }
  return reasons.length === 0 ? { ok: true } : { ok: false, reasons };
}
